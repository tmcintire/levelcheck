<template>
  <div>
    <v-container>
      <v-row dense>
        <v-col>
          ...
          <LevelCheckSelectors :levels="filteredLevels" :roles="roles" />

          <v-row
            v-if="filteredParticipants && filteredParticipants.length > 0"
            class="level-check-container"
          >
            <NumbersBar
          ...
          <v-row
            v-else-if="!levelCheckLevel && levelCheckRole"
            class="pt-12"
            justify="center"
            align="center"
          >
          ...
          <v-row
            v-else-if="!filteredParticipants"
            class="pt-12"
            justify="center"
            align="center"
          >
          ...
          <v-row v-else class="pt-12" justify="center" align="center">
            <h3>
              There are no participants in this level check, please select
              another level or role
            </h3>
          </v-row>
        </v-col>
      </v-row>
    ...
    <v-snackbar
      v-for="(change, index) in tempUndoChangeState"
      :key="index"
      color="cyan darken-2"
      v-model="change.showChange"
    >
  ...
  computed: mapState({
    filteredParticipants: (state: IApplicationState) => {
      if (state.levelCheckLevel) {
        ...
        const filteredArray: IParticipant[] = participantArray
          .filter((p) => {
    ...
    filteredLevels: (state: IApplicationState) => {
      const filteredLevels = Object.entries(state.event.levels)
        .filter((level: [string, ILevel]) => {
    ...
    roles: (state: IApplicationState) => {
      const allRoles = Object.entries(state.event.participants).map(
        (p) => p[1].role
      )
...
export default class LevelChecks extends Vue {
  ...
  private resizeView() {
    ...
    if (el[0]) {
      ...
      const levelContainer = document.getElementsByClassName(
