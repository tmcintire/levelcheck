<template>
  <v-stepper v-model="newEvent">
    <v-stepper-header>
      <v-stepper-step :complete="newEvent > 1" step="1"
        >Event Details</v-stepper-step
      >

      <v-divider></v-divider>

      <v-stepper-step :complete="newEvent > 2" step="2"
        >Level Details</v-stepper-step
      >
  ...
  computed: mapState({
    levels: (state: IApplicationState) => {
      if (state.selectedEvent) {
        return Object.entries(state.selectedEvent.levels)
