<template>
  <div>
    <v-navigation-drawer v-model="drawer" app>
      <v-list dense>
        ...
        <v-list-group prepend-icon="mdi-home" value="true">
          ...
          <v-list-item-content>
            <v-list-item
              @click="handleEventSelection(ev.value)"
              v-for="ev in userEvents"
              :key="ev.value"
              :class="event && ev.value === event.eventId ? 'active' : ''"
            >
    ...
    <v-app-bar app color="indigo" dark>
      ...
      <v-toolbar-title v-if="event"
        >{{ event.name }} Level Check</v-toolbar-title
      >
    </v-app-bar>
  </div>
</template>
